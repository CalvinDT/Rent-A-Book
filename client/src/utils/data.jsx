export const sliderItems = [
    {
      id: 1,
      img: "https://i.ibb.co/LrQLkRL/slide-1.png",      
      title: "A BOOK CAN TAKE YOU PLACES",
      desc: "GET 10% OFF WHEN YOU SUBSCRIBE TO OUR NEWSLETTER.",
      bg: "fbf0f4",
    },
    {
      id: 2,
      img: "https://i.ibb.co/Tr1Lp58/slider2.png",
      title: "THE MORE YOU BUY THE MORE YOU SAVE",
      desc: "GET 10% OFF FOR ANY ADDITIONAL BOOK (UP TO 40%).",
      bg: "fcf1ed",
    },
    {
      id: 3,
      img: "https://i.ibb.co/HYmTSm2/slider-3.png",
      title: "BECOME Rent-A-Book MEMBER",
      desc: "AND ENJOY ALL THE BENEFITS",
      bg: "d6f3f9",
    },
  ];

  

export const categories = [
    {
      id: 1,
      img: "https://i.ibb.co/cg3L378/category-1.png",
      title: "FICTION",
      cat: "fiction"
    },
    {
      id: 2,
      img: "https://i.ibb.co/MDNBvgJ/category-2.png",
      title: "NON-FICTION",
      cat: "non-fiction"
    },
    {
      id: 3,
      img: "https://i.ibb.co/YXrBQyw/category-img.png",
      title: "KIDS",
      cat: "kids"
    },
  ];

  export const bestsellers = [
   
    {
      id:2,
      img:"https://i.ibb.co/d7LM3VN/H-2.jpg",
      authors:"Anna Malaika Tubbs",
      description: "Martin Luther King, Jr., Malcolm X and James Baldwin, three men of one generation who changed history, each of whom acknowledged their debts to their mothers as influences crucial to their paths. And yet, these mothers — Louise, Alberta and Berdis — like so many Black women throughout history, have until now remained largely unknown and unacknowledged. Who were these unsung Black mothers? What were their own journeys and what were the lessons they taught? Intimate and arching in scope, The Three Mothers is celebratory, revelatory and surprising. A remarkable book.",
      title:"The Three Mothers: How the Mothers of Martin Luther King, Jr., Malcolm X, and James Baldwin Shaped a Nation",
      category: "non-fiction", 
      price: "$5.99",
    },
    
    {
      id:7,
      img:"https://i.ibb.co/4YcbjmQ/H-7.jpg",
      authors: "Stuart Woods",
      description: "In this latest adrenaline-charged thriller from #1 New York Times bestselling author Stuart Woods, Stone Barrington takes on a vengeful rival. Stone Barrington is looking forward to some quiet time in New York City, until he is asked to transport precious, top-secret cargo across the Atlantic. Taking on the challenge, Stone flies off unaware of what—or who—he is bringing with him. ",
      title:"A Safe House (Stone Barrington Series #61)",
      category: "fiction", 
      price: "$4.99",
    },
    {
      id:8,
      img:"https://i.ibb.co/LZYhnK4/H-8.jpg",
      authors: "Calla Henkel",
      description: "Two American students abroad find themselves at the center of murder and celebrity. Henkel brilliantly stages a cat and mouse game between a well-known thriller novelist and two art students, one that explores the intensity of female friendships, millennial life, and identity.",
      title:"Other People's Clothes",
      category: "fiction", 
      price: "$2.00",
    },
  
   
    {
      id:11,
      img:"https://i.ibb.co/hVKQ0h8/H-11.jpg",
      authors: "Marcus Pfister, J Alison James (Translator)",
      description: "The Rainbow Fish is an international best-seller and a modern classic. The rainbow fish with its iridescent scales is the most beautiful fish in the whole ocean. But since he is also vain and proud, he becomes more and more lonely over time. Then he realizes that you can’t win friends through beauty. He overcomes his pride and begins to distribute his glitter scales to the other fish.",
      title:"The Rainbow Fish",
      category: "kids",
      price: "$8.95", 
    },
    

  ]

  export const allBooks = [
    {
      id:1,
      img:"https://i.ibb.co/GVWgj8J/H-1.jpg",
      authors:"by Julian Sancton",
      description: "Meticulously researched and realized, with a deep novelistic flare, Madhouse at the End of the Earth reconstructs the action-packed survival story of an early expedition to the South Pole. Amundson, Cook and an inexperienced, undisciplined crew, on an ill-fated ship, imprisoned in the Antarctic ice and darkness. This tale of adventure, excitement and, indeed, terror will captivate those who were drawn to The Lost City of Z, In the Kingdom of Ice and In the Heart of the Sea. Julian Sancton has gifted us an insanely gripping book from start to finish.",
      title:"Madhouse at the End of the Earth: The Belgica's Journey into the Dark Antarctic Night",
      category: "non-fiction",
      price: "$3.99", 
    },
    {
      id:2,
      img:"https://i.ibb.co/d7LM3VN/H-2.jpg",
      authors:"Anna Malaika Tubbs",
      description: "Martin Luther King, Jr., Malcolm X and James Baldwin, three men of one generation who changed history, each of whom acknowledged their debts to their mothers as influences crucial to their paths. And yet, these mothers — Louise, Alberta and Berdis — like so many Black women throughout history, have until now remained largely unknown and unacknowledged. Who were these unsung Black mothers? What were their own journeys and what were the lessons they taught? Intimate and arching in scope, The Three Mothers is celebratory, revelatory and surprising. A remarkable book.",
      title:"The Three Mothers: How the Mothers of Martin Luther King, Jr., Malcolm X, and James Baldwin Shaped a Nation",
      category: "non-fiction", 
      price: "$5.99",
    },
    {
      id:3,
      img:"https://i.ibb.co/1rgn718/H-3.jpg",
      authors: "Tom Standage",
      description: "How often a new lens opens a whole new world! And what a new vista Tom Standage, deputy editor for The Economist, opens up for readers. The whole of human history divided into six eras, each defined by a different beverage. Six glasses: from beer, wine and spirits through coffee, tea and Coca-Cola. Set your course and before you know it you've traveled from ancient Sumer to Greece and Rome, covered the Age of Exploration, Reason and Empire, grasped the impacts of the Industrial Revolution, and ended up in the globalized world of the 21st century. Even a wink at what comes next...",
      title: "A History of the World in 6 Glasses",
      category: "non-fiction", 
      price: "$4.99",
    },
    {
      id:4,
      img:"https://i.ibb.co/xm76Gn7/H-4.jpg",
      authors: "Richard Grant",
      description: "The Deepest South of All is Richard Grant's love letter to his adoptive city of Natchez, Mississippi. Seamlessly blending travelogue, history, memoir, and anecdote, spiced by a good strong dose of irony, any eye for incendiary humor and just plain weirdness. A little Twin Peaks, a lot of Southern Gothic, Grant coaxes out the good, the lovely, the ugly, the sublime and delivers a wholly engaging portrait of this Jewel of the Mississippi,a town with a deep and complicated history.",
      title: "The Deepest South of All: True Stories from Natchez, Mississippi",  
      category: "non-fiction",
      price: "$3.99", 
    },
    {
      id:5,
      img:"https://i.ibb.co/GP88SXV/H-5.jpg",
      authors: "Clare Mackintosh",
      description: "Just when you thought it was safe to board a plane ... Hostage by Clare Mackintosh is one wild ride — a 24 hour flight to be exact. The pilot will NOT need to remind you to return to your seat. You’ll be riveted in place the whole time.",
      title:"Hostage",
      category: "fiction", 
      price: "$4.99",
    
    },
    {
      id:6,
      img:"https://i.ibb.co/NFDYSW9/H-6.jpg",
      authors: "Frank Herbert",
      description: "Arguably a masterpiece of the Science Fiction genre, Dune laid the foundation for many of the epics that followed its publication. Herbert addresses overarching themes of family politics, religion, environment, and the all-encompassing temptation of power, all set against the backdrop of a futuristic civilization that, at times, can be unflinchingly prophetic. You will taste the sand in your mouth and soon be hungry for the next installment.",
      title:"Dune",
      category: "fiction", 
      price: "$4.99",
    },
    {
      id:7,
      img:"https://i.ibb.co/4YcbjmQ/H-7.jpg",
      authors: "Stuart Woods",
      description: "In this latest adrenaline-charged thriller from #1 New York Times bestselling author Stuart Woods, Stone Barrington takes on a vengeful rival. Stone Barrington is looking forward to some quiet time in New York City, until he is asked to transport precious, top-secret cargo across the Atlantic. Taking on the challenge, Stone flies off unaware of what—or who—he is bringing with him. ",
      title:"A Safe House (Stone Barrington Series #61)",
      category: "fiction", 
      price: "$4.99",
    },
    {
      id:8,
      img:"https://i.ibb.co/LZYhnK4/H-8.jpg",
      authors: "Calla Henkel",
      description: "Two American students abroad find themselves at the center of murder and celebrity. Henkel brilliantly stages a cat and mouse game between a well-known thriller novelist and two art students, one that explores the intensity of female friendships, millennial life, and identity.",
      title:"Other People's Clothes",
      category: "fiction", 
      price: "$2.00",
    },
    {
      id:9,
      img:"https://i.ibb.co/tJcrv73/H-9.jpg",
      authors: "Audrey Penn",
      description: "School is starting in the forest, but Chester Raccoon does not want to go. To help ease Chester's fears, Mrs. Raccoon shares a family secret called the Kissing Hand to give him the reassurance of her love any time his world feels a little scary. Since its first publication in 1993, this heartwarming book has become a children's classic that has touched the lives of millions of children and their parents, especially at times of separation, whether starting school, entering daycare, or going to camp. It is widely used by kindergarten teachers on the first day of school. Stickers at the back will help children and their parents keep their Kissing Hand alive.",
      title:"The Kissing Hand",
      category: "kids", 
      price: "$2.99",
    },
    {
      id:10,
      img:"https://i.ibb.co/N3zWShk/H-10.jpg",
      authors: "Shel Silverstein",
      description: "School is starting in the forest, but Chester Raccoon does not want to go. To help ease Chester's fears, Mrs. Raccoon shares a family secret called the Kissing Hand to give him the reassurance of her love any time his world feels a little scary. Since its first publication in 1993, this heartwarming book has become a children's classic that has touched the lives of millions of children and their parents, especially at times of separation, whether starting school, entering daycare, or going to camp. It is widely used by kindergarten teachers on the first day of school. Stickers at the back will help children and their parents keep their Kissing Hand alive.",
      title:"The Giving Tree",
      category: "kids",
      price: "$2.99", 
    },
    {
      id:11,
      img:"https://i.ibb.co/hVKQ0h8/H-11.jpg",
      authors: "Marcus Pfister, J Alison James (Translator)",
      description: "The Rainbow Fish is an international best-seller and a modern classic. The rainbow fish with its iridescent scales is the most beautiful fish in the whole ocean. But since he is also vain and proud, he becomes more and more lonely over time. Then he realizes that you can’t win friends through beauty. He overcomes his pride and begins to distribute his glitter scales to the other fish.",
      title:"The Rainbow Fish",
      category: "kids",
      price: "$8.95", 
    },
    {
      id:12,
      img:"https://i.ibb.co/bsXfDbm/H-12.jpg",
      authors: "by Drew Daywalt, Oliver Jeffers (Illustrator)",
      description: "The hilarious, colorful #1 New York Times bestselling phenomenon that every kid wants! Gift a copy to someone you love today. Poor Duncan just wants to color. But when he opens his box of crayons, he finds only letters, all saying the same thing: His crayons have had enough! They quit! Blue crayon needs a break from coloring all those bodies of water. Black crayon wants to be used for more than just outlining.  And Orange and Yellow are no longer speaking—each believes he is the true color of the sun. What can Duncan possibly do to appease all of the crayons and get them back to doing what they do best?",
      title:"The Day the Crayons Quit",
      category: "kids",
      price: "$4.99", 
    },
    {
      id:13,
      img:"https://i.ibb.co/F8xsBNN/H-13.jpg",
      authors: "by Dolly Parton and James Patterson, Dolly Parton, James Patterson",
      description: "James Patterson AND Dolly Parton in the same room is enough to give pause. But the thought of them writing a book together? We can just about hear Dolly’s voice come through here as she teams up with Patterson for their writing duo debut. A lot of suspense plus the life of a musician and the songs that make your heart ache. We’ve already put the needle on the record, flipped it over AND then flipped it over AGAIN. Repeat plays. Repeat reads.",
      title:"Run Rose Run",
      category: "fiction",
      price: "$2.99", 
    },
    {
      id:14,
      img:"https://i.ibb.co/dMnPbLk/H-14.jpg",
      authors: "by Greer Hendricks, Sarah Pekkanen",
      description: "A psychological thriller here that will have you gnawing at your fingernails asking, WHAT were they all thinking?!. Simply riveting — as such suspense should be.",
      title:"The Golden Couple",
      category: "fiction",
      price: "$3.99", 
    },
    {
      id:15,
      img:"https://i.ibb.co/RPscMBx/H-15.jpg",
      authors: "by Arthur C. Brooks",
      description: "From Strength to Strength: Finding Success, Happiness, and Deep Purpose in the Second Half of Life",
      title:"The Day the Crayons Quit",
      category: "non-fiction",
      price: "$4.30", 
    },

  ]